...
export function MyUpdates() {
    ...
    return <Section>
        <h2>Your updates</h2>
        <List>
            { messages.map( message => <UpdateRow key={ message.id }>
                <Meta>
                    ...
                    { message.sync_status ? <span>{ message.sync_status }</span> : null }
                </Meta>
                ...
                { editing_id === message.id ? <>
                    ...
                    <Actions>
                        ...
                        <Button type="button" variant="danger" disabled={ is_saving } onClick={ () => delete_update( message ) }>
            ...
            { messages.length === 0 ? <p>No submitted updates yet.</p> : null }
        </List>
    </Section>
}
